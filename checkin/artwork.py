    ...
    async def _restore_checkin_background(
        ...
        try:
            ...
            if await self._blacklist_reason_for_illust(
                ...
                logger.warning(
                    f"{LOG_PREFIX} 签到背景恢复被黑名单策略拒绝: "
                    f"illust_id={record.background_illust_id}"
                )
                return replace(saved, mode="fallback")
            if not self._filter_rating([illust]):
                logger.warning(
                    f"{LOG_PREFIX} 签到背景恢复被分级放行策略拒绝: "
                    f"illust_id={record.background_illust_id}"
                )
                return replace(saved, mode="fallback")
        ...
        *,
        ...
        for page_attempt in range(1, CHECKIN_BACKGROUND_PAGE_ATTEMPTS + 1):
            ...
            try:
                ...
                logger.warning(
                    f"{LOG_PREFIX} 签到背景黑名单检查不可用，使用占位图: "
                    f"error_type={type(exc).__name__}"
                )
                return None
        ...
        for idx, illust in enumerate(ordered[:8], 1):
            ...
            try:
                ...
                # 标签黑名单/作品黑名单读取失败时 fail-closed：不放过该候选，
                # 跳过去试下一个；若所有候选都不可用则回退占位图。
                logger.warning(
                    f"{LOG_PREFIX} 签到背景黑名单检查不可用，跳过作品: "
                    f"illust_id={illust_id} error_type={type(exc).__name__}"
                )
                continue
            if reason:
                logger.debug(
                    f"{LOG_PREFIX} 签到背景跳过: reason=blacklist_policy "
