...
const state = {
  ...
  rankingType: "today",
  trendDays: 7,
  safety: { rating_policy: "all", rating_label: "", rating_options: [], custom_terms: [] },
  blacklist: [],
  thumbs: {},
  members: [],
  memberQuery: "",
...
async function reloadAll() {
  ...
  try {
    ...
    if (safetyResult.status === "fulfilled") {
      ...
      failures.push(errorMessage(safetyResult.reason, "内容分级与黑名单数据读取失败"));
...
function renderOverview() {
  ...
  const metrics = [
    ...
    ["标签黑名单", formatCount(data.custom_term_count)],
...
function renderSafety() {
  ...
  const ratingPolicy = state.safety.rating_policy || "all";
  const ratingLabel = state.safety.rating_label || "未知";
  els.customCount.textContent = `${custom.length} 项`;
  els.ratingCount.textContent = ratingPolicy;
  els.policyRating.textContent = ratingLabel;
  els.ratingBadge.textContent = ratingLabel;

  const options = Array.isArray(state.safety.rating_options)
    ? state.safety.rating_options
    : [];
  els.ratingSelect.innerHTML = options.map((option) =>
    `<option value="${escapeHtml(option.value)}"${option.value === ratingPolicy ? " selected" : ""}>${escapeHtml(option.label)}</option>`
  ).join("") || `<option value="${escapeHtml(ratingPolicy)}">${escapeHtml(ratingLabel)}</option>`;
...
function renderSafetyError() {
  ...
  els.ratingCount.textContent = "读取失败";
  els.policyRating.textContent = "读取失败";
  els.ratingBadge.textContent = "读取失败";
  els.customTerms.innerHTML = '<div class="empty error">标签黑名单暂时无法读取。</div>';
}
...
async function reloadSafety() {
  try {
    ...
    renderSafetyError();
    showGlobalError([errorMessage(error, "内容分级与黑名单数据读取失败")]);
    throw error;
  }
}
...
function bindEvents() {
  ...
  els.ratingForm.addEventListener("submit", async (event) => {
    ...
    const policy = els.ratingSelect.value;
    els.ratingError.textContent = "";
    setButtonBusy(submitButton, true, "正在应用…", "应用分级放行");
    try {
      await apiPost("content-safety/rating", { rating_policy: policy });
      showToast("分级放行策略已更新，立即生效");
      await reloadSafety();
    } catch (error) {
      els.ratingError.textContent = error.message || "应用失败，请重试。";
      showToast(els.ratingError.textContent, "error");
    } finally {
      setButtonBusy(submitButton, false, "正在应用…", "应用分级放行");
    }
  });
  ...
  $("termForm").addEventListener("submit", async (event) => {
    ...
    setButtonBusy(submitButton, true, "正在添加…", "添加黑名单标签");
    try {
      await apiPost("content-safety/terms/add", { term });
      input.value = "";
      showToast("黑名单标签已成功添加");
      await reloadSafety();
    } catch (error) {
      ...
      setButtonBusy(submitButton, false, "正在添加…", "添加黑名单标签");
    }
  });
