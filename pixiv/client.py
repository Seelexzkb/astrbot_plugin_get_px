...
class PixivClient:
    ...
    async def search(self, tag: object = "", offset: int = 0) -> list[dict]:
        word = self._normalize_search_word(tag)
    ...
    @staticmethod
    def _normalize_search_word(tag: object) -> str:
        """Pixiv 搜索词：多个标签用空格分隔，按「同时包含」语义检索。"""
        if isinstance(tag, (list, tuple)):
            return " ".join(
                str(item).strip() for item in tag if str(item or "").strip()
            )
