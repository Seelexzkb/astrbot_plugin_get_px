...
class LoliconClient:
    ...
    @property
    def _r18_param(self) -> str:
        """Lolicon r18 参数：0=仅普通，1=仅 R18，2=混合；非法值回退为混合。"""
        value = str(self.r18 or "")
    ...
    async def fetch(
        ...
        *,
        ...
        if tags:
            # Lolicon 的 tag 参数用 `&` 连接表示「且」，`|` 表示「或」。
            params.append(("tag", "&".join(tags)))
        if aspect_ratio:
    ...
    async def search(
        self, tag: object = "", *, count: int = 20, aspect_ratio: str = ""
    ) -> list[dict[str, Any]]:
    ...
    @staticmethod
    def _normalize_tag(tag: object) -> list[str]:
        items = tag if isinstance(tag, (list, tuple)) else [tag]
        return [
