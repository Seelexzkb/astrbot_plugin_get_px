...
def parse_search_tags(value: object) -> list[str]:
    """把搜索标签串拆成多个标签，空格/逗号/顿号分隔，用于「同时包含所有标签」的 AND 检索。"""
...
class SearchMixin:
    ...
    def _any_image_source_available(self) -> bool:
        if self._ordered_image_sources():
            return True
        return bool(getattr(self, "client", None) and self.client.api)

    def _ensure_client_or_error(self, event: AstrMessageEvent) -> bool:
        if self._any_image_source_available():
            return True
        self._init_client()
        return self._any_image_source_available()

    def _ordered_image_sources(self) -> list[str]:
        """按配置返回本次尝试的图片源顺序（不含 Pixiv 回退）。"""
        sources: list[str] = []
        lolicon_client = getattr(self, "lolicon_client", None)
        nyan_client = getattr(self, "nyan_run_client", None)
        if lolicon_client and lolicon_client.available:
            sources.append("lolicon")
        if nyan_client and nyan_client.available:
            sources.append("nyan_run")
        mode = self._cfg_str("image_source_mode", "lolicon_first")
        if mode == "nyan_first" and len(sources) == 2:
            sources.reverse()
        elif mode == "random" and len(sources) > 1:
    ...
    async def _fetch_source_candidates(
        ...
        *,
        ...
        """按配置顺序尝试 Lolicon / Nyan.run，全部失败后回退 Pixiv。多标签按 AND 语义检索。"""
        tags = parse_search_tags(tag)
        search_tag = " ".join(tags)
        for source in self._ordered_image_sources():
            source_key = (
                self._source_key(search_tag, source) if tags else f"{source}:random"
            )
            try:
                if source == "lolicon":
                    client = getattr(self, "lolicon_client", None)
                    if client is None or not client.available:
                        continue
                    if tags:
                        illusts = await client.search(
                            tags, count=count, aspect_ratio=aspect_ratio
                        )
                    else:
                        illusts = await client.random(
                            count=count, aspect_ratio=aspect_ratio
                        )
                else:  # nyan_run
                    client = getattr(self, "nyan_run_client", None)
                    if client is None or not client.available:
                        continue
                    if tags:
                        illusts = await client.search(tags, count=count)
                    else:
                        illusts = await client.random(count=count)
            except Exception as exc:
                logger.info(
                    f"{LOG_PREFIX} {_search_source_label(source)} 请求失败，"
                    f"尝试下一图片源: "
                    f"tag_configured={'yes' if tags else 'no'} "
        ...
        pixiv_source_key = self._source_key(search_tag, "pixiv") if tags else "pixiv:recommended"
        page_offset = offset
        ...
        try:
            if tags:
                illusts = await self.client.search(tags, offset=page_offset)
    ...
    async def _handle_search(
        ...
        illusts = self._filter_rating(illusts)
        if not illusts:
            yield event.plain_result(
                "😶 没有符合当前年龄分级放行策略的作品，"
                "可在插件配置或管理中心的「内容分级与黑名单」中调整"
            )
            return
        ...
        try:
            ...
            yield event.plain_result("🚫 黑名单服务暂不可用，本次请求已拒绝")
            return
        if not illusts:
            yield event.plain_result(
                "😶 可用作品都被标签黑名单过滤了，换个标签后再试"
            )
            return
