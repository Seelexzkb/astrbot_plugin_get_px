...
class PluginWebApi:
    ...
    def register(self) -> None:
        routes = (
            ...
            (
                ...
                self.content_safety,
                ["GET"],
                "Get rating policy and tag blacklist",
            ),
            (
                "content-safety/terms/add",
                self.content_safety_term_add,
                ["POST"],
                "Add tag blacklist term",
            ),
            (
                "content-safety/terms/remove",
                self.content_safety_term_remove,
                ["POST"],
                "Remove tag blacklist term",
            ),
            (
                "content-safety/rating",
                self.content_safety_rating_update,
                ["POST"],
                "Update rating allowance policy",
            ),
            (
                "image-blacklist",
    ...
    async def content_safety(self):
        if self.plugin.image_index is None:
            return self._unavailable("标签黑名单数据尚未初始化")
        try:
            custom_terms = await self.plugin.image_index.list_safety_terms()
            policy = normalize_rating_policy(
                self.plugin._cfg_str("rating_policy", DEFAULT_RATING_POLICY)
            )
            return jsonify(
                {
                    "success": True,
                    "rating_policy": policy,
                    "rating_label": RATING_POLICIES[policy]["label"],
                    "rating_options": [
                        {"value": value, "label": spec["label"]}
                        for value, spec in RATING_POLICIES.items()
                    ],
                    "custom_terms": custom_terms,
                }
            )
        except Exception as exc:
            return self.internal_error("读取内容分级与标签黑名单", exc)

    async def content_safety_rating_update(self):
        payload = await self._request_json_object()
        if payload is None:
            return jsonify({"success": False, "error": "请求内容必须是对象"}), 400
        raw_policy = str(payload.get("rating_policy") or "").strip()
        if raw_policy not in RATING_POLICIES:
            return jsonify(
                {
                    "success": False,
                    "error": "无效的分级放行策略，可选值: general / r18 / all",
                }
            ), 400
        config = getattr(self.plugin, "config", None)
        if config is None:
            return jsonify({"success": False, "error": "插件配置不可用"}), 500
        try:
            config["rating_policy"] = raw_policy
            save_config = getattr(config, "save_config", None)
            if callable(save_config):
                save_config()
        except Exception as exc:
            return self.internal_error("保存分级放行策略", exc)
        lolicon_client = getattr(self.plugin, "lolicon_client", None)
        if lolicon_client is not None:
            lolicon_client.r18 = self.plugin._lolicon_r18_param()
        nyan_client = getattr(self.plugin, "nyan_run_client", None)
        if nyan_client is not None:
            nyan_client.r18 = self.plugin._nyan_run_r18_param()
        logger.info(
            f"{self.log_prefix} 分级放行策略已更新: "
            f"rating_policy={raw_policy}"
        )
        return jsonify(
            {
                "success": True,
                "rating_policy": raw_policy,
                "rating_label": RATING_POLICIES[raw_policy]["label"],
            }
        )
    ...
    async def content_safety_term_add(self):
        ...
        try:
            ...
            return self.internal_error("添加标签黑名单", exc)
    ...
    async def content_safety_term_remove(self):
        ...
        try:
            ...
            if not removed:
                return jsonify({"success": False, "error": "标签黑名单中不存在该词"}), 404
            return jsonify({"success": True, "term": term})
        except ValueError as exc:
            return jsonify({"success": False, "error": str(exc)}), 400
        except Exception as exc:
            return self.internal_error("删除标签黑名单", exc)
